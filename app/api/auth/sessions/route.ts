...
import { listSessionsPaged } from "@/services/session.service";

export async function GET(request: NextRequest) {
  try {
    await requirePermission(PERMISSIONS.AUTH_SESSIONS_READ);
    ...
    const { rows, total } = await listSessionsPaged(page, pageSize, userId);
